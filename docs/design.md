...
`sh -c '<body>'` is the same `allow`-speaks-for-everything problem in its purest form: the body is one opaque token, so the hook knows nothing about it — and knowing nothing is precisely the case where vouching is indefensible. Measured, `cat in.txt; sh -c 'cat /outside'` returned `allow`. It now defers. Note what is *not* claimed: the body is still unparsed, and a kill or an outside read inside one still goes unseen. Suppressing a groundless `allow` is worth shipping on its own, separately from earning the right to make a real judgment about the body — which would mean judging paths in bodies that don't run on this host at all, since `docker exec … sh -c 'cat /var/lib/…'` names a path inside a container.
